...
namespace ITnnovative.AOP.Processing.Editor
{
    ...
    {
        public static string AssemblyName = "proj.dll";
        public static readonly Predicate<TypeDefinition> IncludeTypeCondition = s => s.Namespace == "DefaultNamespace";
        ...
        public static void WeaveAssembly(AssemblyDefinition assembly)
        {
            ...
            var dir = Path.GetDirectoryName(AppDomain.CurrentDomain.GetAssemblies().First(a => a.FullName.Contains("UnityEngine")).Location);
                    ...
                    if (instr.OpCode == OpCodes.Ldfld)
                    {
                        ...
                        if (instr.Operand is FieldDefinition)
                        {
                            ...
                            if (opObj.Name == evt.Name)
                            {
                                ...
                                newMethodBody.Add(Instruction.Create(method.IsStatic
                                    ? OpCodes.Ldnull
                                    : OpCodes.Ldarg_0));
                ...
                nameof(AOPProcessor.OnMethod))
        {
            ...
            newMethodBody.Add(Instruction.Create(method.IsStatic
                ? OpCodes.Ldnull
                : OpCodes.Ldarg_0)); // ldnull / ldarg_0
        ...
        public static void WeaveAssemblies(bool isPlayer)
        {
            var unityDefaultAssembly = AssemblyDefinition.ReadAssembly(
                Application.dataPath + $"/../Library/{(isPlayer ? "Player" : "")}ScriptAssemblies/{AssemblyName}",
                new ReaderParameters() {ReadWrite = true, ReadSymbols = true});

            AspectAdder.AddAspects(unityDefaultAssembly);
